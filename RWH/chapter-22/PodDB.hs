...
convPodcastRow x = error $ "Cant convert podcast row " ++ show x 
...
getPodcastEpisodes dbh pc = do 
  ...
  where
    convEpisodeRow [svId, svURL, svDone] = 
        Episode { epId = fromSql svId, epURL = fromSql svURL, epDone = fromSql svDone, epCast = pc}
